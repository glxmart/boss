...
{
  "network": {
    "allowed_hosts": ["api.stripe.com", "api.sendgrid.com", "registry.npmjs.org", "github.com"]
  }
}
```
...
```json
{
  ...
  "setup_commands": ["apt-get update", "apt-get install -y git curl build-essential"],
  ...
  "network": {
    "allowed_hosts": ["registry.npmjs.org", "github.com", "api.anthropic.com"]
  }
}
```
...
```json
{
  ...
  "secrets": ["GITHUB_TOKEN=op://glx/github/token", "VERCEL_TOKEN=op://glx/vercel/token"],
  "network": {
    "allowed_hosts": ["registry.npmjs.org", "github.com", "api.vercel.com"]
  }
}
```
...
```json
{
  ...
  "install_commands": ["npm install -g pnpm", "pnpm install", "pnpm prisma generate"],
  "environment_variables": {
...
```json
{
  ...
  "secrets": ["GITHUB_TOKEN=op://glx/github/token"],
  "network": {
...
async function beforeImplementation() {
  ...
  const missing = requiredSecrets.filter((secret) => !configuredSecrets.includes(secret));
