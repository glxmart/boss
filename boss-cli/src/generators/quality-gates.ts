...
export async function generateQualityGates(
  ...
  const gatesConfig = {
    ...
    git_hooks: {
      pre_commit: {
        enabled: true,
        commands: quality === 'startup' 
          ? ['lint-staged', 'typecheck']
          : ['lint-staged', 'typecheck', 'test-affected']
      },
      commit_msg: {
        enabled: true,
        type: quality === 'enterprise' ? 'conventional-commits-with-ticket' : 'conventional-commits'
      }
    },
    ci: {
      enabled: true,
      checks: quality === 'startup'
        ? ['typecheck', 'lint', 'test']
        : quality === 'production'
        ? ['typecheck', 'lint', 'test', 'coverage', 'security-scan']
        : ['typecheck', 'lint', 'test', 'coverage', 'mutation-test', 'security-scan', 'dependency-check']
    }
  };
