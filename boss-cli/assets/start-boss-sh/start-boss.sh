...
# Configuration
DEBUG=false
INIT_PROMPT="Check .boss/project-config.json initialization status. If initialization.stage is not 'ready', automatically complete the initial setup in this EXACT order: 1) Check if GitHub remote exists using 'git remote -v', 2) If no remote exists: create repository if needed using GitHub MCP (GitHub MCP is already authenticated using GITHUB_PERSONAL_ACCESS_TOKEN - use it directly), ask user for preferences (private/public, organization), add remote using HTTPS URL format 'git remote add origin https://github.com/<owner>/<repo>.git' (NEVER use SSH git@github.com format - always use HTTPS), 3) FIRST push empty main branch: 'git push -u origin main' (main is EMPTY - only empty commit, no validation needed, git will use GITHUB_TOKEN from environment automatically, which is set to same value as GITHUB_PERSONAL_ACCESS_TOKEN). The pre-push hook allows empty main push without validation checks. Main branch must be pushed first (empty) before feature branch. 4) THEN switch to feature branch: 'git checkout feature/boss-initial-setup' (branch should already exist from bootstrap), 5) CRITICAL - BEFORE pushing feature branch: Verify all validation checks pass by running 'pnpm typecheck', 'pnpm lint', 'bash scripts/security-check.sh', and 'pnpm test:unit' (or npm equivalents). Fix any failures before pushing. 6) Push feature branch: 'git push -u origin feature/boss-initial-setup' (this contains all bootstrap files - git will use GITHUB_TOKEN from environment automatically, which is set to same value as GITHUB_PERSONAL_ACCESS_TOKEN). If push fails due to validation errors, fix the issues and try again. 7) Mark initialization.stage as 'ready' in project-config.json, 8) Commit and push project-config.json changes: 'git add .boss/project-config.json && git commit -m \"chore: update project-config.json\" && git push'. Do this automatically without asking for confirmation to proceed."
